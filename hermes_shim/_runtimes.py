"""Host-side agent CLI probe. Stdlib only.
...
def write_runtime_pins(root: Path, pins: dict[str, str]) -> None:
    path = root / "descriptor" / "runtimes.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(pins, sort_keys=True), encoding="utf-8")
