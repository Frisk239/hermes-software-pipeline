...
`contracts check` delegates in-process to `scripts/check_schemas.py`; `architecture check` runs the standard-library AST import-boundary checker against `src/hermes_pipeline` (stable file/line/rule diagnostics, no `import-linter`). Pyright uses its lockfile-provided `nodejs-wheel-binaries` runtime instead of ambient Node. After the environment is installed, the same checks rerun offline (for example `uv sync --frozen --all-groups --offline` and `uv run --offline python -m hermes_pipeline.cli --version`) with no credentials and no further network access.
